...
Grab the latest DMG from the [Releases](https://github.com/rtuszik/markdown-preview/releases) page.

Builds are currently unsigned, on first launch, right-click the app and choose **Open** (macOS Sequoia additionally requires System Settings → Privacy & Security → **Open Anyway**).
...
```sh
git clone git@github.com:rtuszik/markdown-preview.git
cd markdown-preview
open md-preview.xcodeproj
```

Build and run the `md-preview` scheme. Swift Package Manager will resolve [swift-markdown](https://github.com/swiftlang/swift-markdown) on first build.
...
Version.xcconfig    Marketing version (single source of truth)
```

## Releasing

Releases are built and published by GitHub Actions (`.github/workflows/release.yml`). Add a `CHANGELOG.md` entry for the new version, then:

```sh
./scripts/cut-release.sh 0.0.32
```

The script bumps `Version.xcconfig`, commits, tags `v0.0.32`, and pushes; the workflow runs the tests, builds the DMG, and creates the GitHub release with notes from the changelog.
